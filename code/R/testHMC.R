...
q <- as.vector(c(q.a, q.alpha))

library(numDeriv)
neg_log_post_a_alpha(q = q, others = others)
neg_log_post_grad_a_alpha(q = q, others = others)
grad(func = neg_log_post_a_alpha, x = q, others = others)

library(microbenchmark)
microbenchmark(grad(neg_log_post_alpha, x = q.alpha, others = others), 
               neg_log_post_grad_alpha(q.alpha, others))


# try with joint update for alpha and a
rm(list=ls())
source("./hmc_aux.R")
...
q.a <- matrix(log(10), nknots, nt)
q.b <- matrix(0, nknots, nt)
others <- list(y = y.t, alpha = alpha.t, wz = wz.t, b = transform$inv.logit(q.b), a = exp(q.a))
nkt <- nknots * nt
