...
timings <- matrix(NA, length(these.sets), 3)
for (set in these.sets) {
  ...
  table.file   <- paste("./ss-tables/time-", samp.type, "-", which.y, "-", n, 
                        ".txt", sep = "")
  ...
  ####################################################################
  iters <- 500; burn <- 400; update <- 100; thin <- 1; iterplot <- FALSE
  ...
  #                          thin = 10)
  timings[set, 1] <- fit.gev$minutes * 60
  ...
  #                           start = 1, end = iters - burn, update = update)
  timings[set, 2] <- fit.probit$minutes * 60
