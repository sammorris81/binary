...
for (i in 1:length(files)) {
  ...
  if (knots == 1) {
    ...
    bs.results[[setting]][set, 2]  <- table.set[4, 1]
    auc.results[[setting]][set, 2] <- table.set[4, 2]
    bs.results[[setting]][set, 3]  <- table.set[7, 1]
    auc.results[[setting]][set, 3] <- table.set[7, 2]
  } else if (knots == 2) {
    bs.results[[setting]][set, 1]  <- table.set[2, 1]
    auc.results[[setting]][set, 1] <- table.set[2, 2]
    bs.results[[setting]][set, 2]  <- table.set[5, 1]
    auc.results[[setting]][set, 2] <- table.set[5, 2]
    bs.results[[setting]][set, 3]  <- table.set[8, 1]
    auc.results[[setting]][set, 3] <- table.set[8, 2]
  } else {
    bs.results[[setting]][set, 1]  <- table.set[3, 1]
    auc.results[[setting]][set, 1] <- table.set[3, 2]
    bs.results[[setting]][set, 2]  <- table.set[6, 1]
    auc.results[[setting]][set, 2] <- table.set[6, 2]
    bs.results[[setting]][set, 3]  <- table.set[9, 1]
    auc.results[[setting]][set, 3] <- table.set[9, 2]
  }
}
