...
# log.map <- array(0, dim=c(iters - burn, nrow(s.p), length(settings)))0
gev.map <- array(0, dim=c(2000, nrow(s.p), length(settings)))
pro.map <- array(0, dim=c(2000, nrow(s.p), length(settings)))
log.map <- array(0, dim=c(2000, nrow(s.p), length(settings)))
set <- rep(0, 4)
for (setting in settings) {
  set[setting] <- which(results[[setting]][, 1] - results[[setting]][, 2] == 
                 min(results[[setting]][, 1] - results[[setting]][, 2], na.rm = TRUE))
  filename <- paste("sim-results/", setting, "-", set[setting], ".RData", sep = "")
  load(filename)
  
  cat("Start gev \n")
  gev.map[, , setting] <- pred.spgev(mcmcoutput = fit.gev, x.pred = X.p,
                                     s.pred = s.p, knots = knots, start = 8001, 
