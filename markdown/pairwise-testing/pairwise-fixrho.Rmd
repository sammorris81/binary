---
title: "Pairwise Likelihood Fixed Rho"
author: "Sam Morris"
date: "25 May 2015"
output: pdf_document
---
...
The only difference between the two plots, is the knots used to fit the data.
In the simulated data, the knots are placed on a $12 \times 12$ grid.
In knots 1, the knots are placed on a $10 \times 10$ grid, and in knots 2, the knots are placed on a $15 \times 15$ grid.

```{r plotdata-1, echo=FALSE, fig.width=4, fig.height=4}
plot(knots.1, ylim = c(0, 1), xlim = c(0, 1), 
     main = "simulated dataset - knots 1", xlab="", ylab="")
train1.idx <- which(y[obs] == 1)
test1.idx <- which(y[!obs] == 1) + 750  # need to add 750 to get to the testing
points(s[train1.idx, ], pch = 21, col = "dodgerblue4", bg = "dodgerblue1")
points(s[test1.idx, ], pch = 21, col = "firebrick4", bg = "firebrick1")

plot(knots.2, ylim = c(0, 1), xlim = c(0, 1), 
     main = "simulated dataset - knots 2", xlab="", ylab="")
train1.idx <- which(y[obs] == 1)
test1.idx <- which(y[!obs] == 1) + 750  # need to add 750 to get to the testing
points(s[train1.idx, ], pch = 21, col = "dodgerblue4", bg = "dodgerblue1")
points(s[test1.idx, ], pch = 21, col = "firebrick4", bg = "firebrick1")
...
cur.seed <- 101
set.seed(cur.seed)
fit.logit1 <- spGLM(formula = y.o ~ 1, family = "binomial", coords = s.o,
                    knots = knots.1, starting = starting, tuning = tuning,
...
cur.seed <- cur.seed + 1
set.seed(cur.seed)
fit.logit2 <- spGLM(formula = y.o ~ 1, family = "binomial", coords = s.o,
                    knots = knots.2, starting = starting, tuning = tuning,
                    priors = priors, cov.model = cov.model,
                    n.samples = iters, verbose = verbose,
                    n.report = n.report)
```
...
cur.seed <- cur.seed + 1
set.seed(cur.seed)
fit.probit1 <- probit(Y = y.o, X = X.o, s = s.o, knots = knots.1,
...
cur.seed <- cur.seed + 1
set.seed(cur.seed)
fit.probit2 <- probit(Y = y.o, X = X.o, s = s.o, knots = knots.2,
                     iters = iters, burn = burn, update = update)
```
...
cur.seed <- cur.seed + 1
set.seed(cur.seed)
fit.gev2 <- mcmc(y = y.o, s = s.o, x = X.o, s.pred = NULL, x.pred = NULL,
                 beta.init = results.2$par[3], beta.m = 0, beta.s = 100,
                 xi.init = results.2$par[1], xi.m = 0, xi.s = 0.5,
                 knots = knots.2, beta.tune = 1, xi.tune = 1,
                 alpha.tune = 0.05, rho.tune = 0.05, A.tune = 1,
                 beta.attempts = 50, xi.attempts = 50,
                 alpha.attempts = 200, rho.attempts = 200,
                 spatial = TRUE, rho.init = knots2.h, rho.upper = 9,
...
Here are the iteration plots from the two GEV models. The true values are $\beta_0 = `r data$thresh`$, and $\xi = `r xi.t`$.

### Knot spacing 1

```{r gev1ters-1, echo=FALSE}
par(mfrow=c(1, 2))
plot(fit.gev1$beta, type="l", main=bquote(beta[0]))
plot(fit.gev1$xi, type="l", main=bquote(xi))
```

### Knot spacing 2

```{r gev2iters-1, echo=FALSE}
par(mfrow=c(1, 2))
plot(fit.gev2$beta, type="l", main=bquote(beta[0]))
plot(fit.gev2$xi, type="l", main=bquote(xi))
```
...
bs.log1        <- BrierScore(post.prob.log1, y.validate)
```
...
post.prob.gev1 <- pred.spgev(mcmcoutput = fit.gev1, x.pred = X.p,
                             s.pred = s.p, knots = knots.1,
                             start = 1, end = 10000, update = 500)
bs.gev1 <- BrierScore(post.prob.gev1, y.validate)
```
...
post.prob.gev2 <- pred.spgev(mcmcoutput = fit.gev2, x.pred = X.p,
                             s.pred = s.p, knots = knots.2,
                             start = 1, end = 10000, update = 500)
bs.gev2 <- BrierScore(post.prob.gev2, y.validate)
```
...
```{r fit-2, echo=FALSE, include=FALSE, cache=TRUE}
set.seed(3283)  # data
data <- rRareBinarySpat(x, s = s, knots = knots.t, beta = 0, xi = xi.t,
...
```{r fit-3, echo=FALSE, include=FALSE, cache=TRUE}
set.seed(3283)  # data
data <- rRareBinarySpat(x, s = s, knots = knots.t, beta = 0, xi = xi.t,
...
```{r fit-4, echo=FALSE, include=FALSE, cache=TRUE}
set.seed(3283)  # data
data <- rRareBinarySpat(x, s = s, knots = knots.t, beta = 0, xi = xi.t,
