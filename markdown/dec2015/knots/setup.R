...
set.seed(7483)  # site
ns <- 1300  # 500 train
nsettings <- 3  # storing y in a list
nknots <- 441
...
for (setting in 1:nsettings) {
  simdata[[setting]]$y <- matrix(data = NA, nrow = ns, ncol = nsets)
  simdata[[setting]]$s <- array(runif(2 * nsets * ns), 
                                dim = c(ns, 2, nsets))
  simdata[[setting]]$knots <- array(NA, dim = c(nknots, 2, nsets))
  simdata[[setting]]$x <- matrix(1, ns, 1)
  simdata[[setting]]$hotspots <- vector(mode = "list", length = nsets)
  
  for (set in 1:nsets) {
    if (setting == 1) {
      knots <- expand.grid(x = seq(0, 1, length = 21), y = seq(0, 1, length = 21))
      simdata[[setting]]$knots[, , set] <- as.matrix(knots)
    } else if (setting == 2) {
      these.knots <- sample(x = ns, size = nknots, replace = FALSE)
      simdata[[setting]]$knots[, , set] <- simdata[[setting]]$s[these.knots, , set]
    }
    ...
    if (setting == 1 | setting == 2) {
      data <- rRareBinarySpat(x = simdata[[setting]]$x, 
                              ...
                              knots = simdata[[setting]]$knots[, , set], 
    ...
    if (setting == 3) {
      ...
      Sigma <- simple.cov.sp(D=d, sp.type="exponential", 
                             ...
                             finescale.var=0)
      data <- transform$logit(log.prob) + t(chol(Sigma)) %*% rnorm(ns)
      data <- rbinom(n = ns, size = 1, prob = transform$inv.logit(data))
