...
We compare our method to the spatial probit and logit for mapping the probability of the occurance of two plant species, \emph{Tamarix ramosissima} and \emph{Hedysarum scoparium}, for a 1-km$^2$ study region of PR China \citet{Smith2012}.
...
\emph{Tamarix ramosissima} can be found in $2.84\%$ of the grid cells, and \emph{Hedysarum scoparium} can be found in $0.54\%$ of the grid cells.
We subsample the original data using $n = 100$ or $250$ initial locations for two different sampling design.
...
For all models, knots are placed at all the grid cells in the sample.
Additionally, for all models we only include an intercept term $\beta_0$ in the model, and the prior for the intercept is $\beta_0 \sim$ N$(0, 10)$.
For the spatial GEV model, we first fit a pairwise likelihood to obtain estimates $\hat{\rho}$, and $\hat{\alpha}$ which are used as starting values in the MCMC sampler.
The bandwidth parameter has prior $\log(\rho) \sim$ N$(-3, 0.7)$, and $\alpha$ has a beta prior with mean $\hat{\alpha}$ and standard deviation $0.05$.
As with the simulation study, we fix $\xi = 0$ because we do not include any covariates.

For the spatial probit model, the bandwidth parameter has prior $\log(\rho) \sim$ N$(-1, 2)$, and the prior on the variance term for the random effects is IG$(0.1, 0.1)$ where IG$(\cdot)$ is an Inverse Gamma distribution.
For the spatial logit model, the bandwidth parameter has prior Unif$(0.001, 10)$, the prior on the variance term for the random effects is IG$(1, 1)$.
Both the spatial probit and logit models assume an exponetial covariance structure.
